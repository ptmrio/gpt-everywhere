...
(function () {
    ...
    chrome.storage.sync.get(['gptSettingApikey', 'gptSettingDebug']).then((result) => {
        ...
        let searchPrompt;
        let searchPromptIframeElement;
        let searchPromptIframe;
        let searchPromptIframeDocument;

        const showSearchPrompt = ((context = "") => {

            if (typeof searchPromptIframeDocument === 'undefined') {
                initiateSearchPrompt();
            }
            searchPrompt = searchPromptIframeDocument.getElementById('gpt-search-prompt');
        ...
        const initiateSearchPrompt = () => {
            ...
            searchPromptIframeElement = document.createElement('iframe');
            searchPromptIframeElement.id = 'gpt-search-prompt-iframe';
            searchPromptIframeElement.setAttribute('allow', 'clipboard-write');
            searchPromptIframeElement.setAttribute('allowtransparency', 'true');
...
`;
            ...
            searchPromptIframeDocument.body.appendChild(spinnerElement);

            const spinner = searchPromptIframeDocument.querySelector('#gpt-spinner-wrapper');

            // Create our new version of the fetch function
            const gptFetchWithSpinner = function () {
